...
export default function ReadmePage() {
  ...
  const generate = async () => {
    ...
    try {
      const res = await aiApi.generateReadme({ projectId, repositoryId: selectedRepo.id });
      setReadme(res.data.data?.content || '');
    } catch (err) {
      setError(err.response?.data?.message || 'README generation failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">📖 README Generator</div>
          <div className="page-subtitle">AI-generated project documentation</div>
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          {readme && <button className="btn btn-secondary" onClick={copyToClipboard}>📋 Copy</button>}
          <button className="btn btn-primary" onClick={generate} disabled={loading || !selectedRepo}>
            {loading ? <><span className="spinner" /> Generating...</> : '✨ Generate README'}
          </button>
        </div>
      </div>

      <ProjectSubNav activeTab="readme" />
      ...
      {readme ? (
        ...
        <div className="empty-state card">
          ...
          <p>Click "Generate README" to create documentation for {selectedRepo?.name || 'your repository'}.</p>
          {!selectedRepo && <p style={{ color: 'var(--warning)' }}>Select a repository first.</p>}
        </div>
      )}
    </div>
  );
