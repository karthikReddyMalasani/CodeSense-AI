...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      setGlobalError(msg.includes('not configured') || msg.includes('unavailable')
        ? `${provider} sign-in is currently unavailable.` : msg);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="cs-login-form-card">
            ...
            <form onSubmit={handleSubmit} className="cs-auth-form" noValidate>
              ...
              <div className="cs-forgot-row">
                <button type="button" onClick={() => setGlobalError('Password reset is currently unavailable. Please contact your administrator.')} className="cs-forgot-link" style={{ background: 'none', border: 0, cursor: 'pointer', padding: 0 }}>
                  Forgot password?
                </button>
              </div>
